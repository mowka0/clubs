...
## [TASK-040a] Frontend: панель организатора — форма создания клуба + калькулятор дохода
- **Дата:** 2026-03-11
- **Статус:** done
- **Что сделано:**
  - `frontend/src/api/clubs.ts` — добавлены: `CreateClubRequest` интерфейс, `createClub(data)`, `getMyClubs()`, `calculateRevenue()` методы
  - `frontend/src/pages/OrganizerPage.tsx` — полная реализация:
    - **Dashboard**: список клубов организатора (аватар/инициал, название, участники, цена), skeleton при загрузке, empty state "У вас пока нет клубов", error state + retry
    - **Кнопка "Создать клуб"**: переход к многошаговой форме
    - **3-шаговая форма**:
      - Шаг 1 "Основная информация": название (≤60, counter), город, категория (select), тип доступа (3 toggle-кнопки с описанием)
      - Шаг 2 "Настройки клуба": лимит участников (10-80), цена подписки, **Калькулятор дохода** — 2 плитки (Ваш доход/80% и Комиссия/20%), обновляется в реальном времени
      - Шаг 3 "Описание клуба": описание (≤500, counter), правила, вопрос для заявки (только при `accessType=closed`), аватар (input[file] + FileReader + canvas resize 800px, jpeg 0.8 quality)
    - **StepIndicator**: 3 прогресс-бара
    - **Валидация** каждого шага: название, город (required), лимит 10-80, цена >0, описание required
    - **Loading state** при отправке формы, **SuccessScreen** после создания с кнопками "Открыть клуб" и "Создать ещё один"
  - `npm run build` — BUILD SUCCESSFUL ✅ (362 модуля, OrganizerPage 18.39 kB)
- **Проблемы:** нет
- **Следующие шаги:**
  1. TASK-040b — Дашборд организатора: участники, заявки, события, присутствие, финансы (deps: TASK-040a ✅)
  2. TASK-041 — Flow вступления: Stars оплата, форма заявки, invite-ссылки
